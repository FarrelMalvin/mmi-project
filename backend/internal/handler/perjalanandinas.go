...
func (h *PerjalananDinasHandler) EditPerjalananDinas(c *echo.Context) error {
	idParam := c.Param("id")
	...
	if errParse != nil || id <= 0 {
		return c.JSON(http.StatusBadRequest, dto.ErrorResponse{
			Code:    http.StatusBadRequest,
			Status:  "Bad Request",
			Message: "Format ID PPD tidak valid",
		})
	}

	var req dto.UpdatePPDRequest
	if err := c.Bind(&req); err != nil {
		return c.JSON(http.StatusBadRequest, dto.ErrorResponse{
			Code:    http.StatusBadRequest,
			Status:  "Bad Request",
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, dto.ErrorResponse{
			Code:    http.StatusInternalServerError,
			Status:  "Internal Server Error",
			Message: "Gagal menyimpan perubahan data perjalanan dinas",
		})
	}
