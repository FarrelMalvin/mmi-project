...
func (r *RealisasiBon) GetListRiwayatRealisasiBon(ctx context.Context, page int, limit int, f FilterRBS) ([]model.RBSListView, int64, int64, error) {
	...
	baseQuery := r.db.WithContext(ctx).Table("realisasi_bon_sementaras")

	if f.Tahun > 0 {
		baseQuery = baseQuery.Where("EXTRACT(YEAR FROM realisasi_bon_sementaras.periode_berangkat) = ?", f.Tahun)
	}
	if f.Bulan > 0 {
		baseQuery = baseQuery.Where("EXTRACT(MONTH FROM realisasi_bon_sementaras.periode_berangkat) = ?", f.Bulan)
	}
	...
	if err := baseQuery.Select("COALESCE(SUM(total_realisasi), 0)").Scan(&totalsum).Error; err != nil {
		return nil, 0, 0, err
	}
	...
	err := baseQuery.
		...
		Select(`
			...
			realisasi_bon_sementaras.nomor_bon_sementara,
			users.nama,
			dokumens.nomor_dokumen,
			realisasi_bon_sementaras.periode_kembali,
			realisasi_bon_sementaras.periode_berangkat,
...
func (r *RealisasiBon) GetListRiwayatRealisasiBonByAtasan(ctx context.Context, userID uint, page int, limit int, f FilterRBS) ([]model.RBSListView, int64, int64, error) {
	var listData []model.RBSListView
	var totaldata int64
	var totalsum int64
...
func (r *RealisasiBon) GetListRiwayatRealisasiBonById(ctx context.Context, page int, limit int, userID uint) ([]model.RBSListView, int64, int64, error) {
	...
	return listData, totaldata, totalpage, nil
}
