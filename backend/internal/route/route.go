...
func RegisterAuth(e *echo.Echo, h *handler.AuthHandler, authMiddleware echo.MiddlewareFunc) {
	...
	authGroup.PATCH("/change-password", h.ChangePassword, authMiddleware)
}
...
func RegisterPPDRoutes(e *echo.Echo, h *handler.PerjalananDinasHandler, jwtService *config.JWTService) {
	...
	ppdGroup.POST("", h.CreatePengajuanPerjalananDinas, allowAll)
	...
	ppdGroup.PUT("/:id/edit", h.EditPerjalananDinas, middleware.RequireRoles(jwtService, constant.JabatanHRGA))
...
func RegisterRBSRoutes(e *echo.Echo, h *handler.RealisasiBonsHandler, jwtService *config.JWTService) {
	...
	rbsGroup.PUT("/:id/edit", h.EditRBS, middleware.RequireRoles(jwtService, constant.JabatanHRGA))
...
func RegisterUserRoutes(e *echo.Echo, h *handler.UserHandler, jwtService *config.JWTService) {
	...
	userGroup.GET("/profile", h.GetDataProfile, allowAll)
	userGroup.POST("/create", h.CreateNewUser, middleware.RequireRoles(jwtService, constant.JabatanHRGA))
}
